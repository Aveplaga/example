...
export class FeedbackForm {
	...
	constructor(formSelector, params = {}) {
		this.params = {
			...
			classes: {
				...
				controlValidTextRuEn: 'form__control_valid-text-ru-en',
        ...
        invalidTextRuEn: 'Поле не соответствует формату текста на русском и английском языках',
				invalidFileSize: 'Максимальный размер файла для загрузки превышен',
	...
	controlValidation(control) {
		...
		if (type === 'password') {
			...
			if (!control.classList.contains(this.classes.controlTypePasswordConfirm)) {
				...
				if (password) password.dispatchEvent(new Event('change', {bubbles: true}))
			}
		...
		switch (validation) {
			...
			case 'textRu':
				...
				break
			case 'textRuEn':
				if (control.value && !this.textRuEnValidation(control)) {
					errors.push(this.errorsList.invalidTextRuEn)
				}
				break
			case 'fileSize':
	...
	getControlValidation(control) {
		...
		if (
			control.classList.contains(this.params.classes.controlValidTextRuEn)
		) {
			return 'textRuEn'
		}
	...
	/**
	 * Проверка значения поля - только буквы русского и латинского алфавита
	 * @param {HTMLElement} control - Элемент формы
	 * @return {boolean} Результат проверки
	 */
	textRuEnValidation(control) {
		const pattern = /^[a-zA-Zа-яА-Яёй -]+$/
